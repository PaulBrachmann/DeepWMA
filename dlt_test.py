...
if args.inputLabel is not None:
    ...
    # y_test contains index numbers for names of tracts, y_names are names of tracts
...
if params['bilateral_feature']:
    ...
    x_test, _ = tract_feat.bilateral_X_data(
        x_test)
...
output_multi_tract_predition_mask_path = os.path.join(args.outputDir,
                                                      args.outPrefix + '_multi_tract_specific_prediction_mask.h5')
output_multi_tract_predition_report_path = os.path.join(args.outputDir,
                                                        args.outPrefix + '_multi_tract_prediction_report.h5')
if not os.path.exists(output_multi_tract_predition_mask_path):

    # Load model
    model = load_model(args.inputModel)

    y_prediction, prediction_report, con_matrix = nn_model.predict(model, x_test, y_data=y_test_ground_truth_final,
                                                                   y_name=y_names_in_model, verbose=True)
